...
def contests(_id: str):
    ...
    if _id == 0:
        ...
        for cont in c[1:]:
            ...
            if len(last) == 3:
                last = (
                    last[0].strip()
                    + "\n[#9c9388]"
                    + (last[1].string or last[1].contents[1]).strip()
                    + "[/]"
                )
            else:
            ...
            table.add_row(
                ...
                tds[3].string.strip(),
                (tds[4].contents[0].strip() or "Running")
                + f"\n[{colors['gray']}]"
                + tds[4].span.string.strip()
                + "[/]",
                last,
            )
        ...
        console.print("\n\n", table)
        console.print(
            "[bold green]NOTE:[/] Use `cf contests ID` to view problems of an ongoing contest.\nAnd use `cf parse ID` to parse them.\n"
        )
    else:
        console.log("Fetching contest details...")
        driver = client.driver
        driver.get(f"https://codeforces.com/contest/{_id}")
        contest_page_source = driver.page_source
        if driver.current_url != f"https://codeforces.com/contest/{_id}":
            console.print(f"[bold red]ERROR:[/] Status Code: {r.status_code}")
            return
        if len(driver.page_source) == 0:
            console.print(
                "[bold red]ERROR:[/] The contest has not started yet OR it doesn't exist."
            )
            return

        soup = BeautifulSoup(contest_page_source, "html.parser")
        p_tables = soup.find_all("table", class_="problems")
        if not p_tables:
            console.print("[bold red]ERROR:[/] Unable to parse problems table.")
            return

        problems = p_tables[0].find_all("tr")[1:]
        contest = soup.find_all("table", class_="rtable")[0].find_all("tr")
        ...
        for problem in problems:
            ...
            table.add_row(
                ...
                f"[{colors['gray']}]"
                + problem_details[1].string.strip()
                + "\n"
